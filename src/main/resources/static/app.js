...
const toggleTimelineSection = document.getElementById('toggleTimelineSection');
const timelineSectionBody = document.getElementById('timelineSectionBody');
const toggleStatsSection = document.getElementById('toggleStatsSection');
const statsSectionBody = document.getElementById('statsSectionBody');
...
function setSectionCollapsed(button, body, collapsed, title) {
...
function renderTimeline(days) {
  ...
  days.forEach(day => {
    ...
    summary.textContent = `${formatDate(day.date)}（${day.events.length} 条）`;
    details.appendChild(summary);

    const content = document.createElement('div');
    day.events.forEach(event => content.appendChild(renderEventItem(event)));
...
function renderStats(days) {
  ...
  days.forEach(day => {
    const details = document.createElement('details');
    details.className = 'stats-item';
    details.open = false;

    const summary = document.createElement('summary');
    summary.innerHTML = `<strong>${formatDate(day.date)}</strong>`;
    details.appendChild(summary);
...
toggleTimelineSection.addEventListener('click', () => {
  const collapsed = !timelineSectionBody.classList.contains('hidden');
  setSectionCollapsed(toggleTimelineSection, timelineSectionBody, collapsed, '打卡记录');
});

toggleStatsSection.addEventListener('click', () => {
  const collapsed = !statsSectionBody.classList.contains('hidden');
  setSectionCollapsed(toggleStatsSection, statsSectionBody, collapsed, '按天统计');
});

(async function init() {
  setSectionCollapsed(toggleTimelineSection, timelineSectionBody, false, '打卡记录');
  setSectionCollapsed(toggleStatsSection, statsSectionBody, false, '按天统计');
  await refreshStatus();
  await loadTimeline();
  await loadStats();
})();
